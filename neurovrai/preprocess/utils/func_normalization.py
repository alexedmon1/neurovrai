...
def normalize_func_to_mni152(
    ...
    if registration_method == 'ants':
        # ANTs path: Use antsApplyTransforms with composite transform
        logger.info("Using ANTs transforms...")
        logger.info("Applying func→anat→MNI152 transforms in single step...")
        logger.info("  This may take 2-5 minutes for 4D time series...")

        # ANTs interpolation mapping
        ants_interp_map = {
            'spline': 'BSpline',
            'trilinear': 'Linear',
            'nn': 'NearestNeighbor'
        }
        ants_interp = ants_interp_map.get(interpolation, 'BSpline')

        # antsApplyTransforms applies transforms in reverse order
        # We need: func→anat (BBR), then anat→MNI (composite)
        # ANTs composite file is passed as t1w_to_mni_warp
        apply_cmd = [
            'antsApplyTransforms',
            '-d', '3',
            '-i', str(func_file),
            '-r', str(mni152_template),
            '-o', str(func_normalized),
            '-n', ants_interp,
            '-t', str(t1w_to_mni_warp),  # ANTs composite transform (anat→MNI)
            '-t', str(func_to_anat_bbr)   # BBR transform (func→anat)
        ]

        logger.info(f"  Running antsApplyTransforms...")
        result = subprocess.run(apply_cmd, capture_output=True, text=True)

        if result.returncode != 0:
            logger.error(f"antsApplyTransforms failed: {result.stderr}")
            raise RuntimeError(f"ANTs functional normalization failed")

        logger.info(f"  Normalized functional data saved: {func_normalized}")
        logger.info("")

        # For ANTs, return the composite transform as the warp
        func_to_mni_warp = t1w_to_mni_warp

    else:
        # FSL path: Use convertwarp + applywarp
        logger.info("Using FSL transforms...")
