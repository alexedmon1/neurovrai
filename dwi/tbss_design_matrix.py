...
destination = '/mnt/elysium/IRC805/dti/tbss'

file = pd.read_csv(covfile)
file.set_index('Subject', inplace=True, drop=True)

dti_files = natsorted(glob('/mnt/elysium/IRC805/dti/dtifit/FA/*.nii.gz'))
subjects = []
for i in dti_files:
    s1 = i.split('/')
    s2 = s1[-1].split('_')
...
os.makedirs('/mnt/elysium/IRC805/dti/tbss/'+exposure, exist_ok=True)
dummies.to_csv('/mnt/elysium/IRC805/dti/tbss/'+exposure+'/design.txt', index=False, header=None, sep=' ')

#%% Create file list
d = {}
for item in dummies.index:
        file = '/mnt/elysium/IRC805/dti/dtifit/FA/'+str(item)+'_FA.nii.gz'
        d[item] = file
df = pd.DataFrame.from_dict(d, orient='index')
df[0].to_csv('/mnt/elysium/IRC805/dti/tbss/'+exposure+'/filelist.txt', index=False, header=None)
